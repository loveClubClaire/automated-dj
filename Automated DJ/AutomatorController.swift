...
class AutomatorController: NSObject {
    ...
    func spawnMasterTimer(){
        Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(masterTimerFunction), userInfo: nil, repeats: true)
    }
    ...
    func masterTimerFunction(){
        ...
        let futureTime = Date.init().addingTimeInterval(180)
        ...
        let dateFormatterShowTime = DateFormatter()
        ...
        for aShow in MasterScheduleObject.dataArray {
            if dateFormatterShowTime.string(from: aShow.startDate! as Date) == dateFormatterShowTime.string(from: futureTime){
                ...
                let showStartTimeSeconds = (Int(minuteFormatter.string(from: aShow.startDate! as Date))! * 60)
                let currentTimeSeconds = (Int(minuteFormatter.string(from: Date.init()))! * 60) + (Int(secondFormatter.string(from: Date.init())))!
    ...
    func playGeneratedPlaylist(_ playlistName: String){
        let applescriptBridge = ApplescriptBridge()
        ...
        var delayTime = DispatchTime.now() + Double(Int64( (rawDelayTime - 0.5) * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
        ...
        if applescriptBridge.isiTunesPlaying() == false {
            //In theory, this sets delay time to zero.
